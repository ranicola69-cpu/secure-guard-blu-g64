...
# ── Patch 16: Remove duplicate NativeWorkletsModule codegen spec from react-native-reanimated ──
# react-native-reanimated/src/specs/NativeWorkletsModule.ts generates NativeWorkletsModuleSpecJSI
# which conflicts with the one generated by react-native-worklets.
# Since worklets provides the canonical codegen spec, stub reanimated's version to a plain
# TurboModuleRegistry.get call (no Spec extends TurboModule), preventing codegen from generating
# the conflicting JNI class while keeping the JS runtime import working.
REANIMATED_WORKLETS_SPEC = os.path.join(
    "frontend", "node_modules", "react-native-reanimated",
    "src", "specs", "NativeWorkletsModule.ts"
)
if os.path.exists(REANIMATED_WORKLETS_SPEC):
    with open(REANIMATED_WORKLETS_SPEC, "r") as f:
        spec_content = f.read()
    PATCH16_MARKER = "[patch16]"
    if PATCH16_MARKER not in spec_content:
        STUB_CONTENT = (
            "'use strict';\n"
            "// [patch16] Stubbed: NativeWorkletsModuleSpec is provided by react-native-worklets.\n"
            "// This stub avoids duplicate NativeWorkletsModuleSpecJSI codegen generation.\n"
            "import { TurboModuleRegistry } from 'react-native';\n"
            "import type { TurboModule } from 'react-native';\n"
            "export default TurboModuleRegistry.get<TurboModule>('WorkletsModule');\n"
        )
        with open(REANIMATED_WORKLETS_SPEC, "w") as f:
            f.write(STUB_CONTENT)
        print("[OK] Patch 16: stubbed NativeWorkletsModule spec in react-native-reanimated")
    else:
        print("[--] Patch 16: already stubbed")
else:
